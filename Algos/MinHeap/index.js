...
class MinHeap {
    ...
    extract2() {
        ...
        this.heap.pop();
        let Leif = 1;
        while(this.heap[Leif] > this.heap[Leif*2] || this.heap[Leif] > this.heap[(Leif*2) + 1]){
            this.heap[Leif*2] <= this.heap[(Leif*2)+1]
            ? 
            (this.swap(Leif, Leif*2), Leif = Leif*2) 
            : 
            (this.swap(Leif, (Leif*2)+1), Leif = (Leif*2) + 1)
        }
        return temp;
    }
...
const testArr1 = new MinHeap();
testArr1.insert(1);

// testArr1.printHorizontalTree();
console.log(testArr1.top());